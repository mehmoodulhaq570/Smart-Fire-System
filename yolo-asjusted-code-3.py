...
VERTICAL_OFFSET_CM = 12.7   # Vertical offset between camera and servo center
HORIZONTAL_OFFSET_CM = 0.0  # Horizontal offset (no horizontal offset)

FOCAL_LENGTH = 0.36
KNOWN_WIDTH = 1.0  # Assuming 1.0 cm reference width (adjust if needed)
...
def calculate_pan_angle(x, horizontal_offset=HORIZONTAL_OFFSET_CM):
    ...
    offset_angle = (dx / DISPLAY_SIZE[0]) * CAMERA_HORIZONTAL_FOV
    corrected_angle = 90 + offset_angle + (horizontal_offset / DISPLAY_SIZE[0]) * CAMERA_HORIZONTAL_FOV
    return int(np.clip(corrected_angle, 0, 180))

def calculate_tilt_angle(y, distance, vertical_offset=VERTICAL_OFFSET_CM):
    dy = y - (DISPLAY_SIZE[1] / 2)
    pixel_angle = (dy / DISPLAY_SIZE[1]) * CAMERA_VERTICAL_FOV
    vertical_offset_angle = np.degrees(np.arctan(vertical_offset / max(distance, 1)))
    corrected_angle = 90 + pixel_angle + vertical_offset_angle
    return int(np.clip(corrected_angle, 0, 180))
